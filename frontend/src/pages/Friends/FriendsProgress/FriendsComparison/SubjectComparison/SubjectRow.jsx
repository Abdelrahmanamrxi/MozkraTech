...
const SubjectRow = ({ subject, index, }) => (
  ...
  >
    ...
    <div className="flex items-center gap-3">
      <span className="text-purple-300/50 text-xs w-8 flex-shrink-0">You</span>
      ...
      <span className="text-white font-semibold text-xs w-8 text-right flex-shrink-0">{subject.youScore}%</span>
    </div>
 
    {/* Friend bar */}
    <div className="flex items-center gap-3">
      <span className="text-purple-300/50 text-xs w-8 flex-shrink-0">Sarah</span>
      <div className="flex-1 h-2 bg-[#2A2440] rounded-full overflow-hidden">
        <motion.div
          initial={{ width: 0 }}
      ...
      <span className="text-white font-semibold text-xs w-8 text-right flex-shrink-0">{subject.friendScore}%</span>
     
    </div>
 <p className="text-white font-semibold text-sm">{subject.friendSubject}</p>
    {/* Diff */}
    <div className="flex items-center gap-1.5">
      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#EF4444" strokeWidth="2" strokeLinecap="round">
        <path d="M5 12h14M12 5l7 7-7 7" />
      </svg>
      <span className="text-red-400 text-xs font-semibold">{Math.abs(subject.diff)}%</span>
      <span className="text-purple-300/40 text-xs">behind</span>
    </div>
  </motion.div>
);
