            ...
            {filtered.map((f) => (
              ...
              >
                <div className="relative">
                  <div
                    className={`w-11 h-11 rounded-full ${f.color} flex items-center justify-center text-white font-bold text-sm`}
                  >
                    {f.initials}
                  </div>
                  <span
                    className={`absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full border-2 border-[#1E1B2E] ${f.away ? "bg-yellow-400" : f.online ? "bg-green-400" : "bg-gray-500"}`}
                  />
                </div>
        ...
        <div className="flex flex-col lg:flex-row lg:gap-12 gap-6 items-center w-full">
          ...
          >
            <div className="flex items-center gap-3">
              <div
                className={`w-14 h-14 rounded-xl ${userData.color} flex items-center justify-center text-white font-bold text-lg flex-shrink-0`}
              >
                {userData.initials}
              </div>
              <div>
                ...
                <p className="text-purple-300/60 text-sm">
                  {t("progress.level")} {userData.level} • #{userData.rank}
                </p>
          ...
          >
            <div className="flex items-center gap-3">
              <div className="relative flex-shrink-0">
                <div
                  className={`w-14 h-14 rounded-xl ${friend.color} flex items-center justify-center text-white font-bold text-lg`}
                >
                  {friend.initials}
                </div>
              ...
              <div>
                ...
                <p className="text-purple-300/60 text-sm">
                  {t("progress.level")} {friend.level} • #{friend.rank}
                </p>
                <p className="text-purple-300/40 text-xs">
                  {t("progress.rank")}
                </p>
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-5">
              <StatCard
                icon={<ClockIcon />}
                value={friend.studyHours}
                label={t("progress.studyHours")}
                diff={friend.diff.studyHours}
              />
              <StatCard
                icon={<TargetIcon />}
                value={friend.goals}
                label={t("progress.goals")}
                diff={friend.diff.goals}
              />
              <StatCard
                icon={<YellowFireIcon />}
                value={friend.streak}
                label={t("progress.dayStreak")}
                diff={friend.diff.streak}
              />
              <StatCard
                icon={<BadgeIcon />}
                value={friend.achievements}
                label={t("progress.achievements")}
                diff={friend.diff.achievements}
              />
            </div>
