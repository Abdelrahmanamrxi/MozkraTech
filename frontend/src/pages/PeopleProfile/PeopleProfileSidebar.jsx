...
export default function PeopleProfileSidebar({
  user,
  progressRing,
  itemVariants,
  glassPanel,
  ...
  const isOutgoingPending =
    friendship?.status === "pending" && isOutgoingRequest;
  const isIncomingPending =
    friendship?.status === "pending" && isIncomingRequest;
          ...
          {!isFriendshipPending && (
            ...
            >
              ...
              <p className="mb-3 text-[11px] text-slate-200">
                {t("stats.xpToNext", { xp: user.xpToNext })}
              </p>
              ...
              <div className="mt-2 flex justify-end">
                <span className="text-[11px] font-Inter font-bold text-white/90">
                  {t("stats.complete", {
                    value: Number(user.xpProgress).toFixed(1),
                  })}
                </span>
              </div>
            </div>
