...
export const generateQuizController = asyncHandler(async (req, res, next) => {
  ...
  if (fileExtension === "pdf") {
    ...
    throw new HttpException(
      "File format not supported! Only PDF and PPTX files are allowed",
      400,
    );
  }
...
export const saveQuizProgress = asyncHandler(async (req, res, next) => {
  const { quizId } = req.params;
  const { userAnswers, currentQuestionIndex } = req.body;
