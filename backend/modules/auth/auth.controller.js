...
export const confirmEmail = asyncHandler(async (req, res, next) => {
  ...
  if (
    !user.OTPEmail ||
    !user.OTPEmailExpiresAt ||
    user.OTPEmailExpiresAt < new Date()
  ) {
  ...
  await userModel.updateOne(
    { email },
    { isVerified: true, $unset: { OTPEmail: 0, OTPEmailExpiresAt: 0 } },
  );
...
export const refreshToken = asyncHandler(async (req, res, next) => {
  ...
  if (user.isDeleted) {
    ...
    return next(new HttpException("User account deleted", 401));
  }
...
export const loginWithGoogle = asyncHandler(async (req, res, next) => {
  ...
  return res
    .status(200)
    .json({ message: "Login with Google Is Successful", accessToken });
});
